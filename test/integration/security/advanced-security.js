...
describe(
  ...
  function() {
    ...
    it('should fail to login with a bad user', function(done) {
      testUserClient
        .login({ username: 'naughty', password: '1234' })
        .then(function() {
    ...
    it('should list all users, options with criteria', function(done) {
      adminClient.exchange.security.listUsers(
        '*',
        { criteria: { '_meta.path': '/_SYSTEM/_SECURITY/_USER/_ADMIN' } },
        function(e, users) {
          if (e) return done(e);
    ...
    it('should get a specific user, without rolled up group data, using includeGroups:false', function(done) {
      adminClient.exchange.security.getUser(
        testUserSaved.username,
        { includeGroups: false },
        function(e, user) {
          if (e) return done(e);
    ...
    it('delete a user, fail to access the system with the deleted user', function(done) {
      adminClient.exchange.security.deleteUser(testUserSaved, function(e) {
        ...
        testUserClient
          .login({ username: testUserSaved.username, password: 'PWD-UPD' })
          .then(function() {
            done(new Error('this was not meant to happn'));
          })
          .catch(function(e) {
      ...
      it('can remove group permissions', function(done) {
        adminClient.exchange.security
          ...
          .then(function(fetchedGroup) {
            ...
            return adminClient.exchange.security.removeGroupPermissions(
              groupName,
              removePermissions
            );
          })
            ...
            .then(function() {
              ...
              testUpsertClient.exchange.component.method2(function(e) {
                ...
                adminClient.exchange.security.addGroup(testUpsertGroup1, function(e) {
                  ...
                  adminClient.exchange.security.upsertUser(
                    testUpsertUser,
                    { overwriteMemberships: true },
                    function(e) {
                      expect(!e).to.be(true);
