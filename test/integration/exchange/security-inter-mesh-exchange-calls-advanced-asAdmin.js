          ...
          'service-name': {
            instance: {
              ...
              allowedMethodNotEvent: function($happn, asAdmin, callback) {
                return $happn.events.on('/data/forbidden', () => {}, callback);
              },
          ...
          'service-name': {
            instance: {
              allowedMethodNotOtherRemoteMethod: function($happn, asAdmin, callback) {
                try {
                  if (asAdmin)
                    return $happn.asAdmin.exchange['secureMesh'][
                      'service-name'
                    ].allowedMethodNotOtherMethod(callback);
              ...
              allowedMethodNotEvent: function($happn, asAdmin, callback) {
                if (asAdmin)
                  return $happn.asAdmin.event['secureMesh']['service-name'].on(
                    '/data/forbidden',
                    () => {},
                    callback
                  );

                $happn.event['secureMesh']['service-name'].on(
                  '/data/forbidden',
                  () => {},
                  callback
                );
