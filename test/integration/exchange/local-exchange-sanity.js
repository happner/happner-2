...
describe(testName, function () {
  ...
  it('does not add more than ' + allowedOverhead + '% overhead on local exchange functions', function (done) {
    ...
    function runDirectly() {
      var module = require(path.join(libFolder, testName))();
      module.start({});
  ...
  context('as async with promise', function () {
    ...
    it('can call ok', function (done) {
      mesh.exchange.component.methodOk({key: 'value'})
    ...
    it('can call with error', function (done) {
      mesh.exchange.component.methodError()
        .then(function () {
    ...
    it('can inject $happn into position 1', function (done) {
      mesh.exchange.component.methodInjectHappn1({})
        .then(function (result) {
          result.meshName.should.equal(testName);
          done();
        })
    ...
    it('can inject $happn into position 2', function (done) {
      mesh.exchange.component.methodInjectHappn2({})
        .then(function (result) {
          result.meshName.should.equal(testName);
          done();
        })
    ...
    it('does inject $origin as _ADMIN', function (done) {
      mesh.exchange.component.methodInjectOrigin({key: 'value'})
        .then(function (result) {
          result.should.eql({
            key: 'value',
            meshName: testName,
    ...
    it('can inject $happn last', function (done) {
      mesh.exchange.component.methodInjectHappnLast({key: 'value'})
        .then(function (result) {
          result.should.eql({
            key: 'value',
            meshName: testName,
