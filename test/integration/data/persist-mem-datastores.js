...
describe(path.basename(__filename), function () {
  ...
  var libFolder = path.resolve(__dirname, '../../..') + path.sep + ['test', '__fixtures', 'utils'].join(path.sep);
  ...
  it('writes to mem path, and then to persist path, ensures the data is in the right places', function (done) {
    ...
    service.instance.exchange.persistComponent.writeData({
      ...
      service.instance.exchange.persistComponent.writeData({
        ...
        helper.getRecordFromHappn({instanceName:'persist-mem-datastores', dataPath:'/_data/persistComponent/mem/some/data'}, function(e, record){
