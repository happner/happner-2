...
describe(
  ...
  function() {
    ...
    it('we examine the output of the mesh permissions and ensure that they reflect our module', function(done) {
      adminClient.exchange.security.getSystemPermissions({ nocache: true }, function(
        e,
        permissions
      ) {
        if (e) return done(e);
        ...
        expect(
          permissions.methods['/b4_permissions_translation/SecuredComponent/method1'].authorized
        ).to.be(true);
        expect(
          permissions.methods['/b4_permissions_translation/SecuredComponent/method2'].authorized
        ).to.be(true);
        expect(
          permissions.methods['/b4_permissions_translation/SecuredComponent/method3'].authorized
        ).to.be(true);
    ...
    it('we add a test user has permissions to access none of the ProtectedComponent methods, we test that this works', function(done) {
      var testUserClient;
    ...
    it('we add a test user that belongs to a group that has permissions to access all of the ProtectedComponent methods, we test that this works', function(done) {
      ...
      var testUser = {
        ...
        permissions: {
          methods: {
            '/b4_permissions_translation/SecuredComponent/*': {
              authorized: true,
    ...
    it('we add a test user that belongs to a group that has permissions to access one of the ProtectedComponent methods, we test that this works', function(done) {
      ...
      var testUser = {
        ...
        permissions: {
          methods: {
            '/b4_permissions_translation/SecuredComponent/method2': {
              authorized: true,
              description: 'a test method'
            }
          }
              ...
              function(eventName, methodCB) {
                testUserClient.event.b4_permissions_translation.SecuredComponent.on(
                  ...
                  function(message) {
                    ...
                    if (eventFiredCount < 4) {
                      //because subsequent tests are firing this off as well
    ...
    it('we add a test user that belongs to a group that has permissions to access a protected web route, we test that this works', function(done) {
      ...
      var testUser = {
        ...
        permissions: {
          ...
          web: {
            ...
            '/b4_permissions_translation/SecuredComponent/WebDelete': {
              actions: ['delete'],
              description: 'allow only deletes'
            },
            '/b4_permissions_translation/SecuredComponent/WebAny': {
              actions: ['*'],
      ...
      adminClient.exchange.security.addUser(testUser, function(e, result) {
        ...
        testUserClient
          .login(testUser)
          .then(function() {
            doRequest('/b4_permissions_translation/SecuredComponent/Web', null, 'GET', function(
              response
            ) {
