...
function EndPointInstance(mesh, internals, endpointName, exchangeAPI, eventAPI, happn, endpointConfig){
...
EndPointInstance.prototype.connect = function(callback){
  ...
  async.whilst(function(){return connecting;}, function(attemptCB){
    ...
    var attempCBInterval = function(error){
      ...
      if (error)
        if (_this.endpointConfig.connectionErrorAttemptsLimit > 0)
          if (_this.endpointConfig.connectionErrorAttemptsLimit == attempts) return attemptCB(new Error('connection error attempt limit of ' + attempts + ' reached'));

      if (_this.endpointConfig.connectionAttemptsLimit > 0)
        if (_this.endpointConfig.connectionAttemptsLimit == attempts) return attemptCB(new Error('connection attempt limit of ' + attempts + ' reached'));
    ...
    _this.client.get('/mesh/schema/description', {}, function (error, description) {
      ...
      try {
        ...
        if (_this.endpointName !== description.name) {
          ...
          var errorMessage = 'endpoint ' + _this.endpointName + ' returned description for ' + description.name;
        ...
        _this.update(description, function(e){
          ...
          _this.client.on('/mesh/schema/description',

            _this.update, // <------ the function.

            function (e) {
              if (e) {
                _this.mesh.log.error('failed description subscription to \'%s\'', _this.endpointName, e);
                return attempCBInterval(e);
              }
  ...
  _this.Happn.client.create(endpointConfig, function (error, client) {
    ...
    client.onEvent('reconnect-successful', function (eventData) {
      ...
      process.nextTick(function () {
        ...
        _this.mesh.emit('endpoint-reconnect-successful', {
          endpointConfig: _this.client.__endpointConfig,
          endpointName: _this.client.__endpointName,
          eventData: eventData
        });
...
EndPointInstance.prototype.update = function(description, callback){
  ...
  _this.Internals._updateEndpoint(_this.mesh, _this.endpointName, _this.exchangeAPI, _this.eventAPI, function (e) {
...
EndPointService.prototype.initialize = function (config, callback) {
  ...
  async.eachSeries(Object.keys(config.endpoints), function (endpointName, endpointCB) {
      ...
      if (!endpointConfig.connectionErrorAttemptsLimit) endpointConfig.connectionErrorAttemptsLimit = 10; //connection can fail 10 times before we break

      if (!endpointConfig.connectionAttemptsInterval) endpointConfig.connectionAttemptsInterval = 5000;

      if (!endpointConfig.config.keyPair) endpointConfig.config.keyPair = _this.mesh._mesh.happn.server.services.security._keyPair;

      var endPointInstance = new EndPointInstance(_this.mesh, _this.Internals, endpointName, _this.exchangeAPI, _this.eventAPI, _this.Happn, endpointConfig);
...
EndPointService.prototype.stop = function (options, callback) {
  ...
  if (_this.activeEndpoints && Object.keys(_this.activeEndpoints).length > 0){
    ...
    async.each(Object.keys(_this.activeEndpoints), function(endpointName, endpointCB){
