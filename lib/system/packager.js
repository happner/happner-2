  ...
  // Read the files
    ...
    .then(function () {
      _this.scripts.forEach(function (script) {
        ...
        if (process.env.NODE_ENV == 'production' && !script.min) {
          ...
          script.buf = minifyJS(script.buf.toString()).code;
        }
      });
    })
    ...
    .then(function () {
      _this.scripts.forEach(function (script) {
        ...
        // in one process
        _this.__fileWatchers.push(fs.watch(script.file, {interval: 1000}, function (curr, prev) {
...
Packager.prototype.assemble = function () {
  ...
  return new Promise(function (resolve, reject) {
    ...
    _this.scripts.forEach(function (script, scriptIndex) {
      var scriptStr = script.buf.toString();
      _this.merged.script += scriptStr + '\n';
      console.log('merged:::', scriptIndex);
    });
