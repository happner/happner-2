...
ComponentInstance.prototype.initializeCachedBoundExchange = function(mesh, componentName) {
  this.boundExchangeCache = mesh.happn.server.services.cache.__caches['happner-bound-exchange' + componentName] ||
...
ComponentInstance.prototype.secureDataBoundToOrigin = function(meshData, componentName, origin) {
  ...
  securedMeshData.increment = function(path, gauge, increment, callback) {
    ...
    return meshData.increment(this.getPath(path), gauge, increment, { onBehalfOf:origin.username }, callback);
  };
...
ComponentInstance.prototype._attachRouteTarget = function(mesh, meshRoutePath, componentRoutePath, targetMethod, route) {
  ...
  if (typeof methodDefn !== 'function')
    throw new Error(`Middleware target ${_this.name}:${targetMethod} not a function or null, check your happner web routes config`);
  ...
  this.log.info(`attached web route for component${this.name}: ${meshRoutePath}`);
