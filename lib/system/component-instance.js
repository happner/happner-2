...
ComponentInstance.prototype._getWebOrigin = function (mesh, params) {
  ...
  if (!query[cookieName]) return null;
...
ComponentInstance.prototype._attachRouteTarget = function (mesh, meshRoutePath, componentRoutePath, targetMethod, route) {
  ...
  // 1. ensure web methods are not on exchange
  // 2. advertise web routes in description
  // 3. support multiple methods array
  // 4. support single methods array
  // 5. tag for detatch
  // 6. added support for next in middleware with injections
  // 7. added support for injecting origin from cookie instead of url query string
  //
  // X. document + migration document
  ...
  connect.use(meshRoutePath, serve || methodDefn);
  connect.use(componentRoutePath, serve || methodDefn);

  // tag for _detatch() to be able to remove middleware when removing component

  (serve || methodDefn).__tag = this.name;
  ...
  Object.keys(mesh.config.web.routes).forEach(function (mountRoute) {
    ...
    connect.use(mountRoute, serve || methodDefn);
  });
  ...
  //
  //     if (methodDefn.$happnSeq !== 'undefined') args.splice(methodDefn.$happnSeq, 0, _this);
  //
  //     if (methodDefn.$originSeq){
  //
  //       if (!mesh.config.happn.secure) args.splice(methodDefn.$originSeq, 0, null);
  //       else args.every(function(arg){
  //
  //         if (arg.method != null && arg.url){
  //
  //           try{
  //             var query = require('url').parse(arg.url, true).query;
  //             if (query.happn_token){
  //
  //               var session =  securityService.decodeToken(query.happn_token);
  //               //var session = pubsubService.getSession(decodedToken.id);
  //               if (!session) throw new Error('sesson expired for token ' + query.happn_token);
  //
  //               args.splice(methodDefn.$originSeq, 0, session);
  //             }
  //
  //           }catch(e){
  //             _this.log.warn('failed to map incoming request token to $origin');
  //             args.splice(methodDefn.$originSeq, 0, null);
  //           }
  //           return false;
  //
  //         } else return true;
...
ComponentInstance.prototype._attach = function (config, mesh, callback) {
  ...
  mesh.data.on(subscribeMask, {event_type: 'set'}, function (publication, meta) {
      ...
      _this.operate(method, args, function (e, responseArguments) {
          ...
          mesh.data.set(message.callbackAddress, response, _this.info.happn.options, function (e) {
