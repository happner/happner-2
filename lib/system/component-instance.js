...
//the following methods are not part of the componentInstance prototype
//as we dont want the accessible from app-land code

const clearCachedBoundExchange = function() {
  if (!boundExchangeCache) return;
  return boundExchangeCache.clear();
}

const initializeCachedBoundExchange = function(mesh, componentName) {
  boundExchangeCache = mesh.happn.server.services.cache.new('happner-bound-exchange' + componentName, {
    type: 'LRU',
    cache: {
  ...
  mesh.happn.server.services.security.on('security-data-changed', clearCachedBoundExchange);
}

const getCachedBoundExchange = function(origin) {
  if (!boundExchangeCache) return null;
  return boundExchangeCache.getSync(origin.username, {
    clone: false
  });
}

const setCachedBoundExchange = function(origin, exchange) {
  if (!boundExchangeCache) return exchange;
  boundExchangeCache.setSync(origin.username, exchange, {
    clone: false
  });
  return exchange;
}
...
const bindToOrigin = function(root, mesh, origin) {
  ...
  if (!mesh.config.happn.secure || (!this.config.security || !this.config.security.authorityDelegationOn))
    return this;

  var bound = getCachedBoundExchange(origin);
  ...
  bound = {
    ...
    stats: this.stats,
    origin: origin,
    data: this.secureDataBoundToOrigin(mesh.data, this.name, origin),
    on: this.on,
    emit: this.emit,
    localEventEmitter: this.localEventEmitter,
    ...
    config: this.config,
    tools: this.tools,
    exchange: this.secureExchangeBoundToOrigin(this.exchange, origin),
    event: this.secureEventBoundToOrigin(this.event, origin),
    localEvent: this.localEvent,
    __raiseOnPublishError: this.__raiseOnPublishError,
    __raiseOnPublishOK: this.__raiseOnPublishOK,
    __raiseOnEmitError: this.__raiseOnEmitError,
    __raiseOnEmitOK: this.__raiseOnEmitOK,
    __getSubscribeMask: this.__getSubscribeMask,
    __authorizeOriginMethod: this.__authorizeOriginMethod
  }
  ...
  return setCachedBoundExchange(origin, bound);
}

ComponentInstance.prototype.secureExchangeBoundToOriginMethods = function(component, boundComponent, origin) {
  ...
  Object.keys(component).forEach((methodName) => {
    ...
    if (typeof component[methodName] == 'object') {
      ...
      return this.secureExchangeBoundToOriginMethods(component[methodName], boundComponent[methodName], origin);
    }
...
ComponentInstance.prototype.secureExchangeBoundToOrigin = function(exchange, origin) {
  ...
  Object.keys(exchange).forEach((componentName) => {
    ...
    this.secureExchangeBoundToOriginMethods(exchange[componentName], boundExchange[componentName], origin);
  });
...
ComponentInstance.prototype.secureEventBoundToOrigin = function(event, origin) {
  ...
  Object.keys(event).forEach(function(componentName) {
    ...
    if (event[componentName].__endpoint) return boundEvent[componentName] = Internals._getSubscriber(event[componentName].__endpoint, event[componentName].__domain, componentName, origin.username);
    Object.keys(event[componentName]).forEach(function(subComponentName) {
      if (event[componentName][subComponentName].__endpoint) boundEvent[componentName][subComponentName] = Internals._getSubscriber(event[componentName][subComponentName].__endpoint, event[componentName][subComponentName].__domain, subComponentName, origin.username);
    });
  });
  return boundEvent;
}
...
ComponentInstance.prototype.secureDataBoundToOrigin = function(meshData, componentName, origin) {
  ...
  securedMeshData.on = function(path, options, handler, callback) {
    ...
    if (this.noConnection()) return callback(new Error('client state not active or connected, on:' + path + ', component:' + componentName));
    if (path == '*') path = '**';
  ...
  securedMeshData.off = function(listenerRef, callback) {
    ...
    if (this.noConnection()) return callback(new Error('client state not active or connected, off ref:' + listenerRef + ', component:' + componentName));
  ...
  securedMeshData.offAll = function(callback) {
    ...
    if (this.noConnection()) return callback(new Error('client state not active or connected, offAll, component:' + componentName));
  ...
  securedMeshData.offPath = function(path, callback) {
    ...
    if (this.noConnection()) return callback(new Error('client state not active or connected, offPath:' + path + ', component:' + componentName));
  ...
  securedMeshData.get = function(path, options, callback) {
    ...
    if (this.noConnection()) return callback(new Error('client state not active or connected, get:' + path + ', component:' + componentName));
  ...
  securedMeshData.getPaths = function(path, callback) {
    ...
    if (this.noConnection()) return callback(new Error('client state not active or connected, getPaths:' + path + ', component:' + componentName));
  ...
  securedMeshData.set = function(path, data, options, callback) {
    ...
    if (this.noConnection()) return callback(new Error('client state not active or connected, set:' + path + ', component:' + componentName));
    if (!options) options = {};
    options.onBehalfOf = origin.username;
  ...
  securedMeshData.increment = function(path, gauge, increment, callback) {
    ...
    if (this.noConnection()) return callback(new Error('client state not active or connected, increment:' + path + ', component:' + componentName));
    //TODO: get paths needs to be done by origin
  ...
  securedMeshData.setSibling = function(path, data, callback) {
    ...
    if (this.noConnection()) return callback(new Error('client state not active or connected, setSibling:' + path + ', component:' + componentName));
    //TODO: get paths needs to be done by origin
  ...
  securedMeshData.remove = function(path, options, callback) {
    ...
    if (this.noConnection()) return callback(new Error('client state not active or connected, remove:' + path + ', component:' + componentName));
    if (!options) options = {};
    options.onBehalfOf = origin.username;
...
ComponentInstance.prototype.initialize = function(name, root, mesh, module, config, callback) {
  ...
  initializeCachedBoundExchange(mesh._mesh, this.name);
  ...
  Object.defineProperty(_this, '__authorizeOriginMethod', {
    get: function() {
      return function(methodName, origin, callback) {
        ...
        //skip authority delegation check, if we are not configured to be secure
        if (!mesh._mesh.config.happn.secure || (!this.config.security || !this.config.security.authorityDelegationOn))
        ...
        }, origin.username, function(e, originSession) {
          if (e) return callback(e);
          mesh._mesh.happn.server.services.security.authorize(originSession, permissionPath, 'set', function(e, authorized) {
            if (e) return callback(e);
            if (!authorized) return callback(mesh._mesh.happn.server.services.error.AccessDeniedError('unauthorized', 'request on behalf of: ' + origin.username));
            callback();
          });
        });
      }.bind(_this);
  ...
  if (!cached || !this.description) {
    ...
    if (this.config.web && this.config.web.routes) {
      ...
      Object.keys(routes).forEach(function(routePath) {
        ...
        if (_this.name == 'www' && routePath == 'global') return;

        if (_this.name == 'www') {
          if (routePath == 'static') routePath = '/';
          else routePath = '/' + routePath;
        } else if (routePath == 'resources' && _this.name == 'resources') routePath = '/' + routePath;
        else if (routePath == 'static') routePath = '/';
    ...
    if (this.config.events) this.description.events = JSON.parse(JSON.stringify(this.config.events));
...
ComponentInstance.prototype._inject = function(methodDefn, parameters, origin) {
  ...
  if (typeof methodDefn.$happnSeq !== 'undefined') parameters.splice(methodDefn.$happnSeq, 0, this.bindToOrigin(origin));
...
ComponentInstance.prototype._loadModule = function(module) {
  ...
  Object.defineProperty(this, 'operate', {
    value: function(methodName, parameters, callback, origin, exchangeSession) {
      _this.__authorizeOriginMethod(methodName, origin, function(e) {
        ...
        try {
          ...
          if (typeof methodDefn !== 'function')
            throw new MeshError('Missing ' + _this.name + '.' + methodName + '()', {
...
ComponentInstance.prototype.__createSetOptions = function(originId, options) {
  ...
  if (this.config.directResponses) return _.merge({
...
ComponentInstance.prototype._attach = function(config, mesh, callback) {
  ...
  _this.emitLocal = function(key, data, callback) {
    ...
    mesh.data.set(eventKey + key, data, {
      noStore: true,
      noCluster: true,
      meta: {
        componentVersion: _this.module.version
            ...
            client.set(callbackAddress, response, options, function(e) {
              if (e) {
                ...
                if (e.message && e.message == 'client is disconnected')
                  return _this.log.warn(logMessage + ':client is disconnected');
                _this.log.error(logMessage, e);
              }
            });
          };
