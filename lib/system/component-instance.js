...
ComponentInstance.prototype.secureData = function(meshData, componentName) {
  ...
  securedMeshData.on = function(path, parameters, handler, callback) {
    ...
    if (this.noConnection())
      return callback(
        new Error(
          "client state not active or connected, on:" +
            path +
            ", component:" +
            componentName
        )
  ...
  securedMeshData.offAll = function(callback) {
    if (this.noConnection())
      return callback(
        new Error(
          "client state not active or connected, offAll, component:" +
            componentName
        )
      );
  ...
  securedMeshData.get = function(path, parameters, callback) {
    ...
    if (this.noConnection())
      return callback(
        new Error(
          "client state not active or connected, get:" +
            path +
            ", component:" +
            componentName
        )
  ...
  securedMeshData.set = function(path, data, options, callback) {
    ...
    if (this.noConnection())
      return callback(
        new Error(
          "client state not active or connected, set:" +
            path +
            ", component:" +
            componentName
        )
...
ComponentInstance.prototype.initialize = function(
  name,
  root,
  mesh,
  module,
  ...
  if (typeof (defaults = module.instance.$happner) == "object") {
    if (defaults.config && defaults.config.component) {
      Object.keys((defaults = defaults.config.component)).forEach(function(
        key
      ) {
  ...
  Object.defineProperty(this.info.happn, "address", {
    ...
    get: function() {
      ...
      try {
        address.protocol =
          mesh._mesh.config.happn.services.transport.config.mode;
      } catch (e) {
  ...
  if (!cached || !this.description) {
    ...
    if (this.config.web && this.config.web.routes) {
      ...
      Object.keys(routes).forEach(function(routePath) {
        ...
        _this.description.routes[routePath].type =
          route == "static" ? "static" : "mware";
      });
    }
    ...
    for (var methodName in this.module.instance) {
      ...
      if (
        !this.config.schema ||
        (this.config.schema && !this.config.schema.exclusive)
      ) {
...
ComponentInstance.prototype._defaultParameters = function(
  method,
  methodSchema
) {
...
ComponentInstance.prototype._getWebOrigin = function(mesh, params) {
  ...
  try {
    cookieName =
      mesh.config.happn.services.connect.config.middleware.security.cookieName;
  } catch (e) {} //do nothing
...
ComponentInstance.prototype._runWithInjection = function(
  args,
  mesh,
  methodDefn
) {
...
ComponentInstance.prototype._attachRouteTarget = function(
  ...
  var methodDefn =
    typeof targetMethod == "function"
      ? targetMethod
      : this.module.instance[targetMethod];
...
ComponentInstance.prototype.__createSetOptions = function(originId, options) {
  if (this.config.directResponses)
    return _.merge({ targetClients: [originId] }, options);
  else return options;
};
...
ComponentInstance.prototype._attach = function(config, mesh, callback) {
  ...
  _this.emit = function(key, data, options, callback) {
    ...
    var eventKey =
      "/_events/" + _this.info.mesh.domain + "/" + _this.name + "/";
  ...
  _this.emitLocal = function(key, data, callback) {
    ...
    mesh.data.set(
      ...
      data,
      {
        noStore: true,
        noCluster: true,
        meta: { componentVersion: _this.module.version }
      },
      callback
    );
  };
  ...
  var listenAddress =
    "/_exchange/requests/" + this.info.mesh.domain + "/" + this.name + "/";
        ...
        function(e, responseArguments) {
          ...
          var reply = function(
            ...
            if (callbackPeer) {
              ...
              try {
                client =
                  mesh.happn.server.services.orchestrator.peers[callbackPeer]
                    .client;
              } catch (e) {
          ...
          if (
            _this.serializer &&
            typeof _this.serializer.__encode == "function"
          ) {
          ...
          _this.log.$$TRACE(
            "operate( reply( RESULT %s",
            message.callbackAddress
          );

          var options = _this.__createSetOptions(
            publication.origin.id,
            _this.info.happn.options
          );
          reply(
...
ComponentInstance.prototype._detatch = function(mesh, callback) {
  ...
  mesh.data.offPath(subscribeMask, function(e, r) {
    if (e)
      _this.log.warn(
        "half detatched, failed to remove request listener %s",
        subscribeMask,
        e
      );
    callback(e);
