...
ComponentInstance.prototype.originBindingNecessary = function(mesh, origin){
  ...
  if (!origin || origin.username == '_ADMIN')
    return false;
  //not a secure mesh:
  if (!mesh.config.happn.secure)
    return false;
  //authority delegation not set up on component, and not set up on the server
  if ((!this.config.security || this.config.security.authorityDelegationOn == null) && !mesh.config.authorityDelegationOn)
    return false;
...
const bindToOrigin = function(root, mesh, origin) {
  ...
  bound = {
    asAdmin:this.asAdmin,//TODO asAdmin must be on component api, in case we are not binding
    name: this.name,
    stats: this.stats,
    origin: origin,
