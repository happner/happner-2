...
ComponentInstance.prototype.describe = function(cached) {
  ...
  for (var methodName of methodNames) {
    ...
    if (!this.config.schema || (this.config.schema && !this.config.schema.exclusive)) {
      ...
      this._configureParameters(method, methodDefined);
      continue;
    }
...
ComponentInstance.prototype._inject = function(methodSchema, parameters, origin) {
  let cloned = parameters.slice();
  parameters.splice(
    0,
    parameters.length,
    ...methodSchema.parameters
      ...
      .map(argName => {
        if (argName === '$happn') {
          return this.bindToOrigin(origin);
        }
        if (argName === '$origin') {
          return origin;
        }
        ...
        try {
          ...
          if (callback) {
            if (methodSchema.type === 'sync-promise') {
              let result;
              try {
                _this._inject(methodSchema, parameters, origin);
...
ComponentInstance.prototype._configureDefinedMethodSchema = function(
  methodSchema,
  $happnIndex,
  $originIndex,
  parsedParameters
  ...
  if ($happnIndex > -1) {
    methodSchema.parameters.splice($happnIndex, 0, {
      name: '$happn'
    });
  }
  if ($originIndex > -1) {
    methodSchema.parameters.splice($originIndex, 0, {
      name: '$origin'
    });
  }
  //try fill in any gaps if there are more parsed method parameters than configured ones
  if (methodSchema.parameters.length <= parsedParameters.length) {
    methodSchema.parameters = parsedParameters.map((parameter, index) => {
      return methodSchema.parameters[index] || parameter;
    });
  }
  return methodSchema;
};

ComponentInstance.prototype._configureParameters = function(method, methodSchema) {
  let $happnIndex, $originIndex;
  const parsedParameters = utilities.getFunctionParameters(method).map(function(argName, argIndex) {
    if (argName === '$happn') $happnIndex = argIndex;
    if (argName === '$origin') $originIndex = argIndex;
    return {
      name: argName
    };
  });
  if (methodSchema.parameters != null) {
    //the user is intentionally setting these
    return this._configureDefinedMethodSchema(
      methodSchema,
      $happnIndex,
      $originIndex,
      parsedParameters
  ...
  methodSchema.parameters = parsedParameters;
  return methodSchema;
};
...
ComponentInstance.prototype._runWithInjection = function(args, mesh, methodDefn) {
  ...
  this._inject(this._configureParameters(methodDefn, {}), parameters, origin);
