...
ComponentInstance.prototype.secureExchangeBoundToOrigin = function (exchange, componentName, origin) {
  ...
  for (var moduleName in exchange){
    boundExchange[moduleName] = {};
    for (var methodName in exchange[moduleName]){
      if (typeof exchange[moduleName][methodName] != 'function') continue;
      boundExchange[moduleName][methodName] = exchange[moduleName][methodName].bind({$self:exchange[moduleName][methodName], $origin: origin});
    }
  }
