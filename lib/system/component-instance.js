...
ComponentInstance.prototype._inject = function(methodDefn, parameters, origin) {
  methodDefn.argNames.forEach((argName, argIndex) => {
    if (parameters.length - 1 < argIndex) {
      parameters.push(undefined);
    }
    if (argName === '$happn') {
      parameters.splice(argIndex, 0, this.bindToOrigin(origin));
    }
    if (argName === '$origin') {
      parameters.splice(argIndex, 0, origin);
    }
  });
