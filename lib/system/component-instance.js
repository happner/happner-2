...
ComponentInstance.prototype.initializeCachedBoundExchange = function(mesh, componentName) {
  this.boundExchangeCache = mesh.happn.server.services.cache.new('happner-bound-exchange' + componentName, {
    type: 'LRU',
    cache: {
  ...
  mesh.happn.server.services.security.on('security-data-changed', this.clearCachedBoundExchange.bind(this));
}
...
const bindToOrigin = function(root, mesh, origin) {
  ...
  if (!origin || origin.username == '_ADMIN') return this;
  //not a secure mesh:
  if (!mesh.config.happn.secure) return this;
  //authority delegation not set up on component, and not set up on the server
  if ((!this.config.security || this.config.security.authorityDelegationOn == null) && !mesh.config.authorityDelegationOn) return this;
  //authority delegation explicitly set not to happen for this component
  if (this.config.security && this.config.security.authorityDelegationOn === false) return this;
