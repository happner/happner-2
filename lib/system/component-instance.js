...
const bindToOrigin = function(root, mesh, origin){
  ...
  var bound = {
    ...
    config:this.config,
    tools:this.tools,
    exchange:this.secureExchangeBoundToOrigin(this.exchange, this.name, origin),
    event:this.event,
...
ComponentInstance.prototype.secureExchangeBoundToOriginMethods = function(component, boundComponent, origin){
  for (var methodName in component){
...
ComponentInstance.prototype.secureExchangeBoundToOrigin = function (exchange, componentName, origin) {
  const boundExchange = {};
  var _this = this;
  Object.keys(exchange).forEach(function(componentName){
    boundExchange[componentName] = {};
    if (exchange[componentName].__localComponent) //local component
      return _this.secureExchangeBoundToOriginMethods(exchange[componentName], boundExchange[componentName], origin);

    Object.keys(exchange[componentName]).forEach(function(subComponentName){
      boundExchange[componentName][subComponentName] = {};
      _this.secureExchangeBoundToOriginMethods(exchange[componentName][subComponentName], boundExchange[componentName][subComponentName], origin);
    });
  });
