(function(isBrowser) {
  var Promisify, Messenger, MeshError, semver;
  var Internals = {};
  ...
  Internals.getEndpoint = function(parameters, api) {
    if (parameters.mesh) {
  ...
  Internals._decoupleEventAPILayer = function(eventAPI) {
    eventAPI.$on = (parameters, handler) => {
      return Internals.getEndpoint(parameters, eventAPI).on(
        parameters.path,
        parameters.options,
        handler
      );
    };
    eventAPI.$once = (parameters, handler) => {
      return Internals.getEndpoint(parameters, eventAPI).once(parameters.path, handler);
    };
    eventAPI.$off = parameters => {
      return Internals.getEndpoint(parameters, eventAPI).off(parameters.id);
    };
    eventAPI.$offPath = parameters => {
      return Internals.getEndpoint(parameters, eventAPI).offPath(parameters.path);
    };
    return eventAPI;
  };
  ...
  Internals._createDecoupledCall = function(exchangeAPI) {
    return parameters => {
      const endpoint = Internals.getEndpoint(parameters, exchangeAPI);
      if (typeof endpoint[parameters.method] !== 'function')
        throw new Error(`unknown exchange method: ${parameters.method}`);
      return endpoint[parameters.method].apply(endpoint, parameters.arguments);
    };
  };
