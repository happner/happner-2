(function(isBrowser) {
  ...
  Internals._updateEndpoint = async function(_this, endpointName, exchangeAPI, eventAPI, callback) {
    ...
    Internals._updateExchangeAPILayer(_this, endpointName, exchangeAPI, function(e) {
      ...
      Internals._updateEventAPILayer(_this, endpointName, eventAPI, function(e, result) {
        if (e) return callback(e);
  ...
  Internals._getSubscriber = function(endpoint, domain, componentName, origin) {
    ...
    return {
      ...
      once: function(key, handler, onceDone) {
        ...
        return endpoint.data.on(eventKey + key, parameters, handler, onceDone);
      },
      ...
      on: function(key, parameters, handler, onDone) {
        ...
        if (origin) parameters.onBehalfOf = origin;
        return endpoint.data.on(eventKey + key, parameters, handler, onDone);
      },

      off: function(key, offDone) {
        if (!offDone) {
          offDone = function(e) {
            if (e) Internals.instance.log.warn("unsubscribe from '%s' failed", key, e);
          };
        }
        if (typeof key === 'number') {
          return endpoint.data.off(key, offDone);
      ...
      offPath: function(path, offDone) {
        if (!offDone) {
          offDone = function(e) {
            if (e) Internals.instance.log.warn("unsubscribe from '%s' failed", path, e);
          };
        }
        return endpoint.data.offPath(eventKey + path, offDone);
      }
    };
  };
