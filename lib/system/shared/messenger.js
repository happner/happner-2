(function(isBrowser) {
  ...
  function Messenger(endpoint, mesh) {
    ...
    endpoint.data.onEvent('connection-ended', function() {
      //clear waiting timeouts of response handlers
      for (var responseHandlerKey in _this.responseHandlers) {
        var responseHandler = _this.responseHandlers[responseHandlerKey];
        clearTimeout(responseHandler.timedout);
        responseHandler.handler(new Error('connection-ended'));
      }
    });

    endpoint.data.onEvent('reconnect-successful', function() {
      // clear response subscriptions containing old session id
      var newSessionId = endpoint.data.session.id;

      Object.keys(_this.__responseHandlerCache).forEach(function(responseMask) {
        var eventId;

        if (responseMask.indexOf(newSessionId) >= 0) return; // dont remove new response paths

        eventId = _this.__responseHandlerCache[responseMask];

        endpoint.data.off(eventId, function(e) {
          if (!e) delete _this.__responseHandlerCache[responseMask];
        });
      });
    });
  }
  ...
  Messenger.prototype.__createCallbackHandler = function(actualHandler, message) {
    ...
    callbackHandler.timedout = setTimeout(
      ...
      _this._endpoint.description.setOptions.timeout || 10000
    );
