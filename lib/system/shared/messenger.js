(function(isBrowser) {
  ...
  function Messenger(endpoint, mesh) {
    ...
    Object.defineProperty(this, '$exchange', { value: this.decoupledEvent() });
    Object.defineProperty(this, '$event', { value: this.decoupledExchange() });
  ...
  Messenger.prototype.decoupledEventValidate = function() {
    //TODOD - validate request
  };

  Messenger.prototype.decoupledEvent = function() {
    return {
      on: (options, handler) => {
        this.decoupledEventValidate(options, handler, 'on');
        let endpoint = options.mesh
          ? this.event[options.mesh][options.component]
          : this.event[options.component];
        return endpoint.on(options.path, handler);
      },
      emit: (options, data) => {
        this.decoupledEventValidate(options, data, 'emit');
        let endpoint = options.mesh
          ? this.event[options.mesh][options.component]
          : this.event[options.component];
        return endpoint.emit(options.path, data);
      }
    };
  };

  Messenger.prototype.decoupledExchangeValidate = function() {
    //TODOD - validate request
  };

  Messenger.prototype.decoupledExchange = function() {
    return {
      call: (options, arguments) => {
        this.decoupledExchangeValidate(options, arguments);
        let endpoint = options.mesh
          ? this.event[options.mesh][options.component]
          : this.event[options.component];
        return endpoint.apply(endpoint, arguments || []);
      }
    };
  };
