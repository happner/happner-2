(function(isBrowser) {
  ...
  Messenger.prototype.__validateMessage = function(methodDescription, args, callback) {
    try {
      ...
      methodDescription.parameters
        .filter(param => param.name !== '$happn' && param.name !== '$origin')
        .map(function(parameterDefinition, index) {
  ...
  Messenger.prototype.__createMessage = function(
    ...
    const message = { callbackAddress, args: [], origin: this.session };

    methodDescription.parameters
      .filter(param => param.name !== '$happn' && param.name !== '$origin')
      .map((parameterDescription, index) => {
        if (parameterDescription.type === 'callback') {
          // Functions as arguments cannot be sent across the network
          ...
          if (!args[index])
            throw new MeshError('Callback for ' + callbackAddress + ' was not defined');

          if (typeof args[index] !== 'function') {
            throw new MeshError(
              'Invalid callback for ' + callbackAddress + ', callback must be a function'
            );
          }
          this.responseHandlers[message.callbackAddress] = this.__createCallbackHandler(
            args[index],
            message
          );
          return;
        ...
        if (typeof args[index] === 'function' && index === args.length - 1) {
          this.responseHandlers[message.callbackAddress] = this.__createCallbackHandler(
            args[index],
            message
          );
    ...
    //   !this.responseHandlers[message.callbackAddress]
    // ) {
    //   this.responseHandlers[message.callbackAddress] = this.__createCallbackHandler(
    //     args[args.length - 1],
    //     message
