;(function (isBrowser) {
  ...
  var Internals, Logger, EventEmitter, clientInstance;
  ...
  if (isBrowser) {
    ...
    HappnClient = require('happn-3').client;
  ...
  MeshClient.prototype.disconnect = function (cb) {
    try {
      if (clientInstance && clientInstance.data){
        if (typeof cb == 'function') return clientInstance.data.disconnect(cb);
        return clientInstance.data.disconnect();
      }
    } catch (e) {
