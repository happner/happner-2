;(function (isBrowser) {
  ...
  var Internals, Logger, EventEmitter, MeshHappnClient;
  ...
  MeshClient.prototype.login = function (credentials, callback) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      setTimeout(function PendLoginAttemptForEmitterSubscriptions() {
        ...
        ['username', 'password', 'secret','token'].forEach(function (key) {
          if (credentials[key]) {
  ...
  MeshClient.prototype.disconnect = function (opts, cb) {
    ...
    try {
      ...
      if (_this.clientInstance && _this.clientInstance.data){

        if (typeof cb == 'function') return _this.clientInstance.data.disconnect(opts, cb);

        return _this.clientInstance.data.disconnect(opts);
      }

      if (typeof cb == 'function') return setImmediate(cb);

    } catch (e) {
      if (_this.log) _this.log.warn('client disconnection failed: ', e);
    }
  };
  ...
  var initialize = function (instance, opts, callback) {
    ...
    return new Promise(function(resolve, reject){
      ...
      if (instance.data && [1,4,5,6,7].indexOf(instance.data.status) >= 0) instance.data.disconnect(createClient);

      else createClient();

      function createClient(){

        MeshHappnClient.create({config: config, info: opts.info, Logger: opts.log, reconnect: opts.reconnect}, function (e, fbclient) {
