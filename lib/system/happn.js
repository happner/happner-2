    ...
    config.happn.services.subscription = {
      config:{
        buckets:[
          ...
          {
            name: 'sets',//using the default implementation (backwards compatible)
            channel:'SET@*'//any sets
          },
          {
            name: 'removes',//using the default implementation (backwards compatible)
            channel:'REMOVE@*'//any removes
          }
        ]
      }
    }
    console.log('setup subscription buckets:::', config.happn.services.subscription);
  }
