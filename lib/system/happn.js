...
module.exports.create = function (mesh, happnServer, config, callback) {
  ...
  if (config.happn.secure) {
    ...
    if (!config.happn.services.security.config.adminUser.password) {
      ...
      if (config.happn.adminPassword) config.happn.services.security.config.adminUser.password = config.happn.adminPassword;

      else {
        // we dont want to make this 'happn' in a production environment
        if (process.env.NODE_ENV === 'production') return callback(new Error('admin password has not been configured on a secure production environment'));
  ...
  if (config.happn.dbfile) return callback(new Error('dbfile is no longer a valid config setting, please use filename'));

  if (utils.getNestedVal(config, 'happn.services.data.config.filename') != null) config.happn.filename = utils.getNestedVal(config, 'happn.services.data.config.filename');
  ...
  if (config.happn.persist) {
    ...
    try {
      ...
      if (config.happn.compactInterval) config.happn.services.data.config.compactInterval = config.happn.compactInterval;
      ...
      log.warn('failure setting up persistence: ' + e.toString());
  ...
  if (dbSetupError) return callback(new Error('failed setting up persistence layer', dbSetupError));

  if (!config.directResponses) {

    if (!config.happn.inboundLayers) config.happn.inboundLayers = [];

    config.happn.inboundLayers.push(function (message, callback) {

      try {

        if (message.request.action == 'on') {

          var path = utils.splitAndCut(message.request.path, '@', 1);

          if (path.match(/^\/_exchange\/responses\/[a-zA-Z0-9-_]*\/[a-zA-Z0-9-_]*\/[a-zA-Z0-9-_]*\/[a-zA-Z0-9-_]{0,30}\*/))
            return callback(new Error('wildcard listens on _exchange responses are forbidden, bad path: ' + path), message);
        }
        callback(null, message);

      } catch (e) {
        callback(new Error('subscription filter failed: ' + e.toString(), e));
      }
    });
  }
  ...
  if (config.happn.inboundLayers || config.happn.outboundLayers) {
    ...
    if (!config.happn.services.protocol.config.inboundLayers) config.happn.services.protocol.config.inboundLayers = [];

    if (!config.happn.services.protocol.config.outboundLayers) config.happn.services.protocol.config.outboundLayers = [];
    ...
    config.happn.services.subscription = {
      config: {
        buckets: [
          {
            name: 'exchange_set',
            channel: '/SET@/_exchange/*'//requests and responses, separate from data
          },
          {
            name: 'events_set',
            channel: '/SET@/_events/*'//inter-component events
          },
          {
            name: 'exchange_remove',
            channel: '/REMOVE@/_exchange/*'//requests and responses, separate from data
          },
          {
            name: 'events_remove',
            channel: '/REMOVE@/_events/*'//inter-component events
          },
          {
            name: 'exchange_all',
            channel: '*@/_exchange/*'//requests and responses, separate from data
          },
          {
            name: 'events_all',
            channel: '*@/_events/*'//inter-component events
          },
          {
            name: 'optimised_set',//for very busy nested type subscriptions, uses strict bucket
            channel: '/SET@/_optimised/*',
            implementation: Happn.bucketStrict
          },
          {
            name: 'optimised_remove',//for very busy nested type subscriptions, uses strict bucket
            channel: '/REMOVE@/_optimised/*',
            implementation: Happn.bucketStrict
          },
          {
            name: 'optimised_all',//for very busy nested type subscriptions, uses strict bucket
            channel: '*@/_optimised/*',
            implementation: Happn.bucketStrict
          }
        ]
  ...
  happnServer.create(config.happn,
    ...
    function (e, happnInstance) {
      ...
      happnInstance.services.session.localAdminClient(clientConfig, function (e, clientInstance) {

        if (e) {
          return happnInstance.stop(function(){
