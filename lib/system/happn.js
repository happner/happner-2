...
module.exports.create = function (mesh, happnServer, config, callback) {
  ...
  const happnLayerInstance = new HappnLayer(config, log);
...
function HappnLayer(config, logger) {
  ...
  if (config.happn.secure) {
    ...
    if (!config.happn.services.security.config.adminUser.password) {
      ...
      if (config.happn.adminPassword) config.happn.services.security.config.adminUser.password = config.happn.adminPassword;
        ...
        if (process.env.NODE_ENV === 'production') throw new Error('admin password has not been configured on a secure production environment');
...
function __initializeDbConfig(config) {
  ...
  if (config.happn.persist) {
    ...
    if (!config.happn.filename) {
      ...
      if (!config.name) throw new Error('persist option is true, but no filename or mesh name specified');
...
function connect(mesh, happnServer, callback) {
  ...
  happnServer.create(this.config.happn,
     ...
     (e, happnInstance) => {
      ...
      happnInstance.services.session.localAdminClient((e, clientInstance) => {
        ...
        this.store.client.set('/test/write', {'can': 'can'}, (e, resp) => {
