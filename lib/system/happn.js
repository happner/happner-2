...
module.exports.create = function (mesh, happnServer, config, callback) {
  ...
  config.happn.inboundLayers.push(function (message, callback) {
    ...
    try {
      ...
      if (message.raw.action == 'on' && message.raw.path.indexOf('/SET@/_exchange/responses') == 0) {
        ...
        if (message.raw.path.indexOf(message.session.id) == -1) {
          return callback(store.server.services.error.AccessDeniedError('wildcard listens on _exchange responses are forbidden, bad path: ' + path));
        }
