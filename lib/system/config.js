...
Config.prototype.process = function (mesh, config, callback) {
  ...
  if (!config.name) config.name = sillyname().split(' ')[0].toLowerCase() + '_' + shortid.generate();
  ...
  // Move component.happn to component.data
  Object.keys(config.components || {}).forEach(function (name) {
    var component = config.components[name];
    if (component.data) return;
    if (!component.happn) return;
    component.data = component.happn;
    delete component.happn;
  });
