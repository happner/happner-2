var shortId = require('shortid'),
  ...
  glob = Promise.promisify(require('glob'), { multiArgs: true }),
...
module.exports.getFunctionParameters = function(fn) {
  ...
  const FN_ARGS = /^(?:async +)?(?:function)?\s*[^\(]*\(\s*([^\)]*)\)/m;
  const FN_ARG_SPLIT = /,/;
...
module.exports.inheritedObjectProps = [
  ...new Set(
    [Object, EventEmitter, Writable, Readable, Duplex, Transform].reduce(
      (result, Clazz) => result.concat(getAllMethodNames(new Clazz())),
      []
    )
  )
];

function getAllMethodNames(toCheck, ignoreInheritedObjectProperties, ignoreNative, ignoreRegex) {
  ...
  return props
    ...
    .filter(fn => {
      if (!ignoreInheritedObjectProperties) return true;
      return this.inheritedObjectProps.indexOf(fn) === -1;
    })
    .filter(fn => {
      if (!ignoreRegex) return true;
      return fn.match(ignoreRegex) == null;
    })
    .filter(fn => {
      if (!ignoreNative) return true;
      const fnString = toCheck[fn].toString();
