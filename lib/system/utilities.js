var shortId = require('shortid'),
  ...
  fs = require('fs'),
  path = require('path'),
  glob = util.promisify(require('glob'), { multiArgs: true }),
...
module.exports.getFunctionParameters = function(fn) {
  ...
  const FN_ARGS = /^(?:async +)?(?:function)?\s*[^\(]*\(\s*([^\)]*)\)|(?:async\s+)?\(?\s*([\w,\s]+).*=>.*/m;
  const FN_ARG_SPLIT = /,/;
  ...
  if (typeof fn === 'function') {
    ...
    (argDecl[1] || argDecl[2]).split(FN_ARG_SPLIT).map(function(arg) {
