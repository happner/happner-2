  ...
  configer.process(_this, config, function (e, instanceConfig) {
    ...
    _this._initializeDataLayer(instanceConfig, function (e) {
      ...
      async.series([
        ...
        function (callback) {
          _this.startupProgress("initialized elements", 89);
          _this.log.$$DEBUG('initialized elements');

          _this._initializePlugins(instanceConfig, callback);
        },
...
Mesh.prototype._createModule = function (spec, callback) {
  ...
  try {
    ...
    var version = utilities.getVersionFromPackageJson(home);
...
Mesh.prototype._initializePlugins = function (config, callback) {
  if (!config.plugins) return callback();
  if (!Array.isArray(config.plugins)) return callback();

  var _this = this;
  async.each(config.plugins, function(pluginFn, callback) {

    pluginFn(_this, _this.log, callback);
