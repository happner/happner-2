...
Mesh.prototype.__getMiddlewareHandlers = function(middleware){
  ...
  if (middleware.methods == null || !middleware.methods.length) throw new Error(`web.middleware.methods empty or null in web config, component: ${middleware.component}`);

  return middleware.methods.map((methodName) => {
    return function(req, res, next) {
      if (!_this.exchange[middleware.component])
        return next(`Component ${middleware.component} not found for global middleware`);

      if (!_this.exchange[middleware.component][methodName])
        return next(`Method ${middleware.component}.${methodName} not found for global middleware`);

      _this.exchange[middleware.component][methodName].call(_this.exchange[middleware.component], req, res, next);
    };
  });
};
