...
Mesh.prototype.describe = function(cached, componentCached) {
  ...
  for (var componentName in this._mesh.elements) {
    ...
    description.components[componentName] = this._mesh.elements[
      componentName
    ].component.instance.describe(componentCached);
  }
...
Mesh.prototype._createElement = util.promisify(function(spec, writeSchema, callback) {
  ...
  async.series(
    [
      ...
      function(done) {
        if (!spec.component.config || !spec.component.config.initMethod) return done();
        if (!_this._mesh.componentsInitialized) return done(); //init will be called when mesh components start initializing
        _this._eachComponentDo(
          {
            methodCategory: 'initMethod',
            targets: [spec.component.name], // only init this component
      ...
      function(done) {
        ...
        if (spec.component.config.dependencies)
          spec.component.dependencies = spec.component.config.dependencies;
        _this._eachComponentDo(
          {
...
Mesh.prototype._addInjectedArgument = function(spec, argName, callback) {
  ...
  for (let fnName of utilities.getAllMethodNames(module, { ignoreInheritedNativeMethods: true })) {
    ...
    if (utilities.functionIsNative(originalFn))
      _this.log.debug(
...
Mesh.prototype._createComponent = function(spec, callback) {
  ...
  componentInstance.initialize(componentName, root, _this, spec.module, componentConfig, function(
    e
  ) {
    if (e) return callback(e);
...
Mesh.prototype._integrateComponent = function(spec, done) {
  ...
  Object.keys(this.exchange).forEach(function(otherComponentName) {
    ...
    if (otherComponentName !== '$call') event[otherComponentName] = _this.event[otherComponentName];

    if (otherComponentName === _this._mesh.config.name) return;

    if (otherComponentName !== '$call')
...
Mesh.prototype.componentStartMethod = function(
  componentName,
  component,
  options,
  calls,
...
Mesh.prototype.deferStartMethod = function(componentName, component, options, calls, call, done) {
  component.instance.on('startup/dependencies/satisfied', () => {
    this.componentStartMethod(componentName, component, options, calls, call, done);
  });
};
  ...
  this._eachComponent(
    ...
    function(componentName, component, done) {
      ...
      if (!config.schema || !config.schema.methods || !config.schema.methods[options.methodName]) {
        ...
        if (options.methodCategory === 'startMethod' && component.dependencies)
          return _this.deferStartMethod(componentName, component, options, calls, call, done);
        return _this.componentStartMethod(componentName, component, options, calls, call, done);
      }
