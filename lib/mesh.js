      ...
      async.series(
        [
          function(cb) {
            ...
            _this.getPackagedModules(instanceConfig, cb);
          },
          function(cb) {
            ...
            _this.loadPackagedModules(instanceConfig, null, cb);
          },
          function(cb) {
...
Mesh.prototype.__getMiddlewareHandlers = function(middleware) {
  ...
  const methods = middleware.methods.map(methodName => {
    return function(req, res, next) {
      ...
      _this.exchange[middleware.component][methodName].call(
        _this.exchange[middleware.component],
        req,
        res,
        next
      );
  ...
  //error handler for failed middleware
  //eslint-disable-next-line
...
Mesh.prototype._updateElement = Promise.promisify(function(spec, callback) {
  return async.series(
    [
      done => {
        this._destroyElement(spec.component.name, done);
      },
      done => {
        this._createElement(spec, true, done);
      }
    ],
    callback
  );
