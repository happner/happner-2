...
Mesh.unsubscribeFromProcessEvents = function (meshInstance, logStopEvent) {
  try {
    ...
    if (Object.keys(root.nodes).length > 0)
      return logStopEvent('info', 'not unsubscribing from process events, root contains nodes: ' + Object.keys(root.nodes).join(',') + ' queued handlers for unsubscription');
    ...
    Object.keys(root.processEventHandlersToRemove).forEach(function(meshName){
      if (root.processEventHandlersToRemove[meshName].__onUncaughtException)
        process.removeListener('uncaughtException', root.processEventHandlersToRemove[meshName].__onUncaughtException);
...
Mesh.prototype._initializeDataLayer = function (config, callback) {
  ...
  DataLayer.create(this, module.exports.AssignedHappnServer, config,
    (err, instance, client) => {
...
Mesh.prototype.getPackagedModules = function (config, callback) {
  ...
  utilities.findInModules('happner.js', module.paths,
    function (e, matches) {
      ...
      async.map(matches, function matchModule(match,cb) {
        ...
        if (typeof suite == 'function') {
          suite(config).then(function (suite) {
...
Mesh.prototype._loadComponentSuites = function (config, callback) {
  ...
  async.map(Object.keys(config.components), function loadComponentSuite(name, cb) {
