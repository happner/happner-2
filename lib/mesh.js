    ...
    _this._initializeDataLayer(instanceConfig, function (e) {
      ...
      async.series([
        ...
        function (callback) {
          ...
          _this._initializeComponents(callback);
        },

        function (callback) {
          _this.startupProgress("initialized components", 89);
  ...
  this.__stopComponents(function (e) {
    ...
    if (e) {
      ...
      /*if (timeout) {
       ...
       console.log(_this._mesh.happn);

       _this._mesh.happn.server.stop(options, function(e) {

       if (e) return _this.log.error('happn stop error', e);

       _this.log.info('happn stopped');

       });

       _this.log.warn('happn not pending');
      ...
      /* _this.log.warn('happn not stopped');
    ...
    _this._stopPlugins(function (e) {
      ...
      _this._endPointService.stop(options, function (e) {
...
Mesh.prototype._initializeComponents = function (callback) {

  var _this = this;

  Object.keys(this.exchange).forEach(function (exchangeComponentName) {

    Object.keys(_this._mesh.elements).forEach(function (componentName) {

      // Assign component instances with own references to exchange and event APIs

      var componentInstance = _this._mesh.elements[componentName].component.instance;
      var exchange = componentInstance.exchange;
      var event = componentInstance.event;

      exchange[exchangeComponentName] = _this.exchange[exchangeComponentName];
      event[exchangeComponentName] = _this.event[exchangeComponentName];

      // Assign each component with all other component's localEvent handlers

      if (!_this._mesh.elements[exchangeComponentName]) return; // it's an endpoint

      var exchangeComponentInstance = _this._mesh.elements[exchangeComponentName].component.instance;
      var localEventEmitter = exchangeComponentInstance.localEventEmitter;
      var localEvent = componentInstance.localEvent;

      localEvent[exchangeComponentName] = localEventEmitter;
...
Mesh.prototype._filterConfig = function (config) {
  ...
  var sharedConfig = {name: config.name, version: this.version, happn: {}, datalayer: {}};
