...
function LoaderProgress(config) {
  ...
  if (!config.splash) config.splash = __dirname + '/index.htm';
...
LoaderProgress.prototype.listen = function(callback) {
  this.__progressServer = http.createServer(function(req, res) {
    if (req.url.indexOf('/loader.htm') === 0) {
      var splashfileStream = require('fs').createReadStream(SPLASH);
      splashfileStream.on('error', function(err) {
        _catchStreamError(err, res);
      });
      return splashfileStream.pipe(res);
    }

    var targetFile = _matchPathRoute(req.url);

    if (targetFile) {
      targetFileStream = require('fs').createReadStream(targetFile);
      targetFileStream.on('error', function(err) {
        _catchStreamError(err, res);
      });
      return targetFileStream.pipe(res);
    }
    ...
    if (req.url === '/log.json') {
      ...
      res.setHeader('Content-type', 'application/json');
      res.write(JSON.stringify(__messageLog), 'binary');
      return res.end();
    }
    ...
    if (PROXY.target === null || PROXY.target === undefined) {
      var noTargetFileStream = require('fs').createReadStream(SPLASH);
      noTargetFileStream.on('error', function(err) {
        _catchStreamError(err, res);
      });
      return noTargetFileStream.pipe(res);
    }
    ...
    req
      ...
      .on('response', function(response) {
        if (response.statusCode === 404) {
          ...
          var responseFileStream = require('fs').createReadStream(SPLASH);
          responseFileStream.on('error', function(err) {
            _catchStreamError(err, res);
          });
          return responseFileStream.pipe(res);
        } else {
...
LoaderProgress.prototype.progress = function(log, progress, error) {
  __loadProgress = progress;
  __progressLog.push({ log: log, progress: progress, error: error });
};
