...
var getProgress = function() {
  ...
  if (!fetchedConfig) {
    promise.get('/config').then(function(error, text) {
      if (error) {
        return;
      }
  ...
  promise.get('/progress').then(function(error, text) {
    if (error) {
      return;
    }
    ...
    if (!latest) latest = { log: 'starting', progress: 0 };

    if (lastProgress !== latest.progress) {
      nanobar.go(latest.progress);
      document.getElementById('progress-log').innerHTML =
        latest.log + '... ' + latest.progress + '% complete';
  ...
  if (qs.verbose === 'true') {
    ...
    promise.get('/log').then(function(error, text) {
      if (error) {
        return;
      }
...
var interval = setInterval(function() {
  try {
    if (promise) {
      promise.get('/ping').then(function(error, text) {
        if (text === 'pong') {
