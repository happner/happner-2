...
Security.prototype.__changePermissions = function(
  $happn,
  entity,
  changePermissions,
  change,
  ...
  let authorized = change === 'remove' ? false : true;
  let permissionsTypes =
    change === 'remove' ? ['methods', 'events', 'web', 'data'] : ['methods', 'events', 'web'];
...
Security.prototype.addGroupPermissions = function($happn, groupName, mergePermissions, callback) {
  this.__validateRequest('addGroupPermissions', arguments, e => {
    ...
    this.getGroup($happn, groupName, (e, group) => {
      if (e) return callback(e);
      this.__changePermissions($happn, group, mergePermissions, 'add', (e, changedGroup) => {
        if (e) return callback(e);
        this.updateGroup($happn, changedGroup, (e, updated) => {
          if (e) return callback(e);
...
Security.prototype.addUserPermissions = function($happn, username, mergePermissions, callback) {
  this.__validateRequest('addUserPermissions', arguments, e => {
    ...
    this.getUser($happn, username, (e, user) => {
      ...
      if (e) return callback(e);
      return this.__changePermissions($happn, user, mergePermissions, 'add', (e, updatedUser) => {
        this.__securityService.users.upsertPermissions(
          this.__transformMeshUser($happn, updatedUser),
          callback
        );
      });
    });
...
Security.prototype.removeGroupPermissions = function(
  ...
  this.__validateRequest('removeGroupPermissions', arguments, e => {
    ...
    this.getGroup($happn, groupName, (e, group) => {
      if (e) return callback(e);
      this.__changePermissions($happn, group, removePermissions, 'remove', (e, changedGroup) => {
        if (e) return callback(e);
        this.updateGroup($happn, changedGroup, (e, updated) => {
          if (e) return callback(e);
          return callback(null, updated);
...
Security.prototype.removeUserPermissions = function($happn, username, removePermissions, callback) {
  this.__validateRequest('removeUserPermissions', arguments, e => {
    ...
    this.getUser($happn, username, (e, user) => {
      if (e) return callback(e);
      return this.__changePermissions(
        $happn,
        user,
        removePermissions,
        'remove',
        (e, changedUser) => {
          if (e) return callback(e);
          this.__securityService.users.upsertPermissions(
            this.__transformMeshUser($happn, changedUser),
            callback
          );
        }
      );
