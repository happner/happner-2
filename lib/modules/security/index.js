...
Security.prototype.__validateRequest = function (methodName, callArguments, callback) {
  ...
  if (['addGroup','updateGroup'].indexOf(methodName) > -1) {
    ...
    if (group.permissions && group.permissions.data && !_this.__validateDataPermissions(group.permissions.data))
      return callback(new Error('data permissions can not start with /_events, /_exchange or /@HTTP'));
  }
...
Security.prototype.__transformMeshPermissions = function ($happn, meshGroupPermissions) {
  ...
  if (meshGroupPermissions.data) {
    Object.keys(meshGroupPermissions.data).forEach(function(permissionPath){
      if (meshGroupPermissions.data[permissionPath].authorized === false)
        return permissions[permissionPath] = {prohibit:meshGroupPermissions.data[permissionPath].actions};
  ...
  for (var permissionPath in meshGroupPermissions.events) {
    ...
    else permissions[this.__getPermissionPath($happn, permissionPath, '_events')] = {
      prohibit: ['on'],
      description: meshGroupPermissions.events[permissionPath].description
    };
  }
  ...
  for (var permissionPath in meshGroupPermissions.web) {
    ...
    var actions = meshGroupPermissions.web[permissionPath].actions;
    var authorized = meshGroupPermissions.web[permissionPath].authorized;

    if (permissionPath[0] != '/') permissionPath = '/' + permissionPath;

    permissions['/@HTTP' + permissionPath] = {};

    if (authorized == null || authorized == true)
      permissions['/@HTTP' + permissionPath].actions = actions;
    else
      permissions['/@HTTP' + permissionPath].prohibit = actions;
  }
...
Security.prototype.__transformHappnPermissions = function ($happn, happnGroupPermissions) {
  ...
  for (var happnPermissionPath in happnGroupPermissions) {
    ...
    if (happnPermissionPath.indexOf('/_events/') == 0) {
      ...
      if (happnPermission.actions && happnPermission.actions.indexOf('on') > -1)
