...
Security.prototype.__createUsersAndGroups = function ($happn, callback) {
  ...
  var createSystemGroups = function (adminUser, addedGroups) {
    ...
    async.eachSeries(['_MESH_ADM', '_MESH_GST'], function (groupName, eachCallback) {
      ...
      _this.__securityService.users.upsertGroup(group, {}, function (e, upsertedGroup) {
        ...
        if (groupName == '_MESH_ADM')
          _this.__securityService.users.linkGroup(upsertedGroup, adminUser, eachCallback);
        else
          eachCallback();
  ...
  _this.__securityService.users.getUser('_ADMIN', {}, function (e, userFound) {
...
Security.prototype.initialize = function ($happn, callback) {
  ...
  try {
    ...
    _this.__attachedToSecurityChanges = false;//backward compatibility

    _this.__attachedToSessionChanges = false;//backward compatibility

    _this.attachToSecurityChanges($happn);

    _this.attachToSessionChanges($happn);
...
Security.prototype.attachToSecurityChanges = function ($happn, callback) {
  ...
  if (_this.__attachedToSecurityChanges){
    if (callback) callback();//backward compatibility
    return;
  }

  _this.__securityService.on('security-data-updated', function(directoryChangedKey){

    _this.__cachedSystemPermissions = null;//reset the cache

    $happn.emit('security-data-updated', directoryChangedKey);
  });

  _this.__securityService.on('security-data-changed', function(directoryChangedKey){
    $happn.emit('security-data-changed', directoryChangedKey);
  });

  _this.__attachedToSecurityChanges = true;
...
Security.prototype.attachToSessionChanges = function ($happn, callback) {
  ...
  if (_this.__attachedToSessionChanges) {
    if (callback) callback();//backward compatibility
    return;
  }
  ...
  _this.__attachedToSessionChanges = true;
...
Security.prototype.listUsers = function ($happn, userName, callback) {
  ...
  this.__validateRequest('listUsers', arguments, function (e) {
    ...
    if (e) return callback(e);
    this.__securityService.users.listUsers(userName, {}, callback);
...
Security.prototype.getUser = function ($happn, userName, callback) {
  ...
  this.__validateRequest('getUser', arguments, function (e) {
    ...
    if (e) return callback(e);
    this.__securityService.users.getUser(userName, {}, callback);
