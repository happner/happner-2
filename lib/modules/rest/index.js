...
Rest.prototype.describe = function($happn, _req, res, $origin) {
  let description = JSON.parse(JSON.stringify(this.__exchangeDescription));
...
Rest.prototype.__respond = function($happn, message, data, error, res, code) {
  ...
  if (error) {
    const errorMessage = utilities.stringifyError(error);
    if (!code) code = 500;
    responseString = responseString.replace('{{ERROR}}', errorMessage);
    $happn.log.error(`rpc request failure: ${errorMessage}`);
  } else {
    if (!code) code = 200;
...
Rest.prototype.__parseBody = function(req, res, $happn, callback) {
  try {
    ...
    this.jsonBody(req, res, (e, body) => {
      if (e) return this.__respond($happn, 'Failure parsing request body', null, e, res);
      callback(body);
    });
  } catch (e) {
    return this.__respond($happn, 'Failure parsing request body', null, e, res);
  }
};
