...
Rest.prototype.login = function ($happn, req, res) {
  ...
  _this.__parseBody(req, res, $happn, function (body) {
      ...
      if (!params.username) return _this.__respond($happn, "Failure parsing request body", null, new Error('no username'), res, 401);
      if (!params.password) return _this.__respond($happn, "Failure parsing request body", null, new Error('no password'), res, 401);

      _this.__securityService.login({username: params.username, password: params.password}, function (e, session) {

        if (e) {
          if (e.toString() == 'AccessDenied: Invalid credentials') return _this.__respond($happn, "Failure logging in", null, e, res, 401);
