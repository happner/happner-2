...
Rest.prototype.__processRequest = function(req, res, body, callPath, $happn, $origin) {
  process.nextTick(() => {
    ...
    if (!component[methodName]) {
      $happn.log.warn(
        `unable to find method: ${componentName}.${methodName} on api id: ${$happn.id}`
      );
      return this.__respond(
        $happn,
        'Failure parsing request body',
...
Rest.prototype.__mapMethodArguments = function(req, res, methodDescription, body, $happn, $origin) {
  ...
  const args = methodDescription.parameters
    //these are injected later in component-instance
    .filter(parameter => parameter.name !== '$happn' && parameter.name !== '$origin')
    .map(parameter => {
...
Rest.prototype.__buildCallMenu = function(exchangeDescription, endpoint, menu) {
  const callMenu = menu || {};
  if (!exchangeDescription.components) {
    return callMenu;
  }
  Object.keys(exchangeDescription.components)
    .filter(componentName => !['security', 'rest'].includes(componentName))
    .forEach(componentName => {
      const component = exchangeDescription.components[componentName];
      Object.keys(component.methods).forEach(methodName => {
        const method = component.methods[methodName];
        let uri = '/' + componentName + '/' + methodName;
        if (endpoint) uri = '/' + endpoint + uri;
        const operation = {
          uri,
          parameters: {}
        };
