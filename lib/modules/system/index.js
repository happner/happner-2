...
function System() {
      ...
      _this.$happn.emitLocal("stats/system", stats, function(e) {
        if (e) {
          if (e.toString() == "Error: client is disconnected")
            return _this.$happn.log.trace(
              "failure to emit stats, internal client not connected"
            );
  ...
  _this.initialize = function($happn, callback) {
    ...
    $happn._mesh.happn.server.services.stats.on(
      "system-stats",
      _this.__handleHappnStats
    );
    ...
    try {
      ...
      this.getDBFileInfo($happn, function(e, fileinfo) {
        ...
        procStats.stats(function(e, result) {
          if (e)
            return $happn.log.error("Failure to fetch cpu usage stats: ", e);
          ...
          for (var componentName in statistics.component) {
            ...
            if (_this.__lastMeasurement) {
              var lastComponent =
                _this.__lastMeasurement.component[componentName];
              ...
              statistics.component[componentName].callsPerSec =
                currentComponent.calls / seconds;
              statistics.component[componentName].emitsPerSec =
                currentComponent.emits / seconds;
              statistics.component[componentName].errorsPerSec =
                currentComponent.errors / seconds;
            }
          }

          if (_this.__lastMeasurement) {
            statistics.callsPerSec =
              (statistics.totalCalls - _this.__lastMeasurement.totalCalls) /
              seconds;
            statistics.emitsPerSec =
              (statistics.totalEmits - _this.__lastMeasurement.totalEmits) /
              seconds;
            statistics.errorsPerSec =
              (statistics.totalErrors - _this.__lastMeasurement.totalErrors) /
              seconds;
          } else {
